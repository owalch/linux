...
void workHard(unsigned low, unsigned high) {
    ...
    unsigned us, j;
    high = high - low;
    rv = ((double)random())/RAND_MAX;	        // random value (0..1) between
    us = low + (unsigned)(rv * high);	        // lower and higher limit	
...
pid_t startWorker(void) {		// make a hard working process
    pid_t pid = fork();
    while (pid == 0) {			// child: pid = 0 -> ifinite loop
        workHard(50, 800);		// work for some time
    }
